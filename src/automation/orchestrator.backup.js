...
export class Orchestrator {
  ...
  async executarCiclo({ segmento, cidade, limite = 20 }) {
    ...
    try {
      ...
      logger.error(`Erro crítico no ciclo: ${e.message}`);
    } finally {
      this.ativo = false;
      logger.info(`📊 Processados: ${this.stats.processados} | Enviados: ${this.stats.enviados} | Erros: ${this.stats.erros}`);
    }
  }
  ...
  // ──────────────────────────────────────────
  //  PROCESSAR RESPOSTA DO CLIENTE (WEBHOOK)
  // ──────────────────────────────────────────
  async processarResposta(numero, texto) {
    logger.info(`\n📩 RESPOSTA de ${numero}`);
    logger.info(`   Mensagem: "${texto.substring(0, 80)}${texto.length > 80 ? '...' : ''}"`);

    // Buscar lead com histórico completo e diagnóstico
    const lead = await db.porWhatsAppCompleto(numero);
    if (!lead) {
      logger.warn(`  ⚠️  Lead não encontrado para ${numero}`);
      return;
    }

    logger.info(`  🏢 Empresa: ${lead.nome_empresa}`);

    // Registrar mensagem recebida
    await db.msg(lead.id, 'recebido', texto);
    await db.update(lead.id, { status: 'respondeu', data_resposta: new Date() });

    // Detectar bot adversário
    const ehBot = this.builder.detectarBot(texto);
    if (ehBot) {
      logger.info(`  🤖 Bot detectado! Aplicando estratégia de bypass...`);
    ...
    // Se explicitamente não quer contato, parar
    if (this.mensagemDeStop(texto)) {
      logger.info(`  🚫 Cliente pediu para parar. Encerrando contato.`);
      await db.update(lead.id, { status: 'descartado' });
      return;
    }

    // Delay humanizado (simula pessoa digitando e pensando)
    const delay = ehBot
      ? 2000 + Math.random() * 3000      // responde rápido para bot
      : 8000 + Math.random() * 15000;    // responde devagar para humano

    logger.info(`  ⏳ Aguardando ${(delay / 1000).toFixed(0)}s para responder...`);
    ...
    if (ok.sucesso) {
      ...
      // Se link de agendamento foi enviado, marcar como quente
      if (resposta.includes(process.env.CALENDLY_LINK)) {
        await db.update(lead.id, { status: 'link_enviado' });
        logger.info(`  🔗 Link de agendamento enviado!`);
      }
    }

    // Atualizar interesse se quente
    if (interesse === 'quente') {
      await db.update(lead.id, { status: 'quente' });
    }
  }

  // ──────────────────────────────────────────
  //  FOLLOW-UP AUTOMÁTICO (para quem não respondeu)
  // ──────────────────────────────────────────
  async executarFollowUp() {
    logger.info('🔄 Executando follow-ups...');

    const leads = await db.buscarParaFollowUp();
    logger.info(`  ${leads.length} leads para follow-up`);

    for (const lead of leads) {
      if (!this.horarioOk()) break;

      const msg = this.builder.construirFollowUp(lead);
      const ok = await this.whatsapp.enviarMensagem(lead.whatsapp, msg);

      if (ok.sucesso) {
        await db.update(lead.id, { status: 'followup_enviado' });
        await db.msg(lead.id, 'enviado', msg);
        logger.info(`  ✓ Follow-up: ${lead.nome_empresa}`);
      }
  ...
  //  UTILITÁRIOS
  // ──────────────────────────────────────────
  mensagemDeStop(texto) {
    const stops = [
      /não me (mande|envie|contate|chame)/i,
      /me (tire|remova) (da lista|daqui|do grupo)/i,
      /para de me (mandar|enviar)/i,
      /não tenho interesse/i,
      /stop|sair|cancelar|descadastrar/i,
      /não quero mais/i,
    ];
    return stops.some(p => p.test(texto));
  }
  ...
  horarioOk() {
    ...
    const allowWeekends = (process.env.ALLOW_WEEKENDS || "false").toLowerCase() === "true";
  if (!allowWeekends && (d === 0 || d === 6)) return false;
    const h = n.getHours();
    const [hi] = (process.env.HORARIO_INICIO || '09:00').split(':').map(Number);
    const [hf] = (process.env.HORARIO_FIM || '17:30').split(':').map(Number);
    return h >= hi && h < hf;
  }
