...
export class WhatsAppClient {
  ...
  parsearWebhook(body) {
    ...
    // Ignorar eventos que no so mensagens recebidas
    ...
    if (data?.message?.buttonsResponseMessage?.selectedDisplayText) {
      texto = data.message.buttonsResponseMessage.selectedDisplayText;
    }

    // Resposta de lista interativa
    if (data?.message?.listResponseMessage?.title) {
      texto = data.message.listResponseMessage.title;
    }

    // Mensagem de template interativo
    if (data?.message?.templateButtonReplyMessage?.selectedDisplayText) {
      texto = data.message.templateButtonReplyMessage.selectedDisplayText;
    }
